...
function MyComponent() {
  ...
  useEffect(() => {
    axios.get(url,
    {
      ...
      .then(response => {
        ...
        const promises = Object.keys(response.data).map((key,index) => fetch(base_url+'/rec/'+key,
         {
  ...
  return (
    <div> 
        <h1 style={{position:"absolute",textAlign:"center",top:"7%",margin:"auto","z-index":"1",width:"100%"}}>Recommended Videos </h1>
       
    <div className="UploadedVideosMain1">
        
       {videos.map((url, index) => (

<div class="card" style={{width: "98%",margin:"10px"}}>
<video className='UploadedVideos1' key={index} src={url[0]}  width='30%'
              height='30%'
              muted = {true} controls />
<div class="card-body">
  <p class="card-title">Description</p>
  <p class="card-text">{url[1]}</p>
  <a href={url[0]} class="btn btn-primary" download >Download</a>
</div>
</div>
